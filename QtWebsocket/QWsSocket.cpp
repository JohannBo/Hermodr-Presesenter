...
void QWsSocket::connectToHost(const QUrl &address, QIODevice::OpenMode mode) {
	...
	if (address.port() < 0) {
		...
		setHostPort(address.port());
	}
	setHostAddress( address.host() );
	setResourceName( address.toString(QUrl::RemoveScheme | QUrl::RemoveAuthority) );
	setOpenMode( mode );
	tcpSocket->connectToHost( address.host(), hostPort(), mode );
}
	...
	switch ( tcpSocketState )
	{
		...
		case QAbstractSocket::ConnectedState:
		{
			if ( wsSocketState == QAbstractSocket::ConnectingState )
			{
				...
				QString handshake = composeOpeningHandShake( _resourceName, hostAddress(), QString(), QString(), key );
